...
planner had been emitting both, so `--automate`, `--reference-audio` and
`--vocal-audio` produced plans this refused *in full*. Both were already working
MCP tools; only the job path was missing. `import_vocal_take` appends a track
without saying so in its name, so it counts as a creation in
