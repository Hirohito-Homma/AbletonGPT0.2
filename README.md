...
設計の詳細は [Architecture](docs/ARCHITECTURE.md)、プロンプト例は [Prompt examples](examples/prompts_ja.md) を参照してください。曲構成を計画・実行するコマンドライン（`arrange` / `jobs` / `arrange-run`）の使い方は [アレンジCLIガイド](docs/CLI_ARRANGE_JA.md) にまとめています。
