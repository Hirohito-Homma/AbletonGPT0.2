...
- `ABLETONGPT_BACKEND`（`remote_script`＝既定、または `extensions`）

Liveバックエンドは既定で Control Surface の Remote Script です。`ABLETONGPT_BACKEND=extensions` にすると、Live 12 Suite Beta 向けの公式 Ableton Extensions SDK コンパニオンへ切り替わります（opt-in・実験的）。両バックエンドは同じ `call` 契約を共有するため、全ツールはそのまま動きます。詳細は [Ableton Extensions SDK 連携](docs/EXTENSIONS_SDK.md) を参照してください。
