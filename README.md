...
- `ABLETONGPT_BACKEND`（`remote_script`＝既定 / `extensions` / `auto`）

Liveバックエンドは既定で Control Surface の Remote Script です。`ABLETONGPT_BACKEND=extensions` にすると、Live 12 Suite Beta 向けの公式 Ableton Extensions SDK コンパニオンへ切り替わります（opt-in・実験的）。`auto` は Extensions を優先し、届かなければ Remote Script へ自動で戻ります（初回の読み取り専用 `ping` で一度だけ確定し、以後は固定。変更系コマンドが両方へ送られて二重適用されることはありません）。両バックエンドは同じ `call` 契約を共有するため、全ツールはそのまま動きます。詳細は [Ableton Extensions SDK 連携](docs/EXTENSIONS_SDK.md) を参照してください。
