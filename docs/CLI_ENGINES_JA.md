...
- `python -m abletongpt.cli.audio tempo` — WAV/AIFF のテンポ(BPM)をオフライン推定（要 `audio` extra: NumPy）

アレンジ計画（`arrange` / `jobs` / `arrange-run`）については [アレンジCLIガイド](CLI_ARRANGE_JA.md) を参照してください。

## 統合エントリポイント `abletongpt-cli`

上記すべてを一つのコマンド `abletongpt-cli <サブコマンド>` からも呼べます。引数はそのまま各CLIへ渡されるため、動作・出力・終了コードは `python -m abletongpt.cli.<name>` と同一です。

```bash
abletongpt-cli --help                 # サブコマンド一覧
abletongpt-cli compose --help         # 各CLI自身のヘルプが出る
abletongpt-cli instruments --genre edm --mood uplifting --json
abletongpt-cli contextual analyze --clip clip.json
abletongpt-cli loudness track.wav --json
```

サブコマンド: `compose` / `contextual` / `instruments` / `expression` / `vocal` / `arrange` / `loudness` / `audio` / `jobs`。単なるディスパッチャなので、ファイルもネットワークもAbletonも触りません。

## `audio` — オーディオ特徴抽出（要 `audio` extra）

WAV/AIFF から特徴量をオフライン抽出します。現状は **tempo（BPM）推定**（オンセット包絡＋自己相関、120 BPM 中心の事前分布でオクターブ誤りを抑制）。決定論的・読み取り専用。DSP に **NumPy** が要るので、この機能だけ opt-in の extra です（`pip install abletongpt[audio]`、または `uv sync --extra audio`）。base インストールは従来どおり stdlib のみ。
...
# 出力例: tempo: 124.98 BPM  (confidence 0.61)   8 s @ 44100 Hz   range 60-200  [onset-autocorrelation]
```

NumPy 未導入で呼ぶと終了コード **3** と「`abletongpt[audio]` を入れて」という明示メッセージになります。key/コード/メロディ抽出は今後の拡張予定です。
